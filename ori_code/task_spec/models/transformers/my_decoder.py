...
class My_decoder(nn.Module):
    def __init__(self, p, in_chans):
        ...
        # 获取dcb31
        self.sup1 = Sup(p, spec, 0, embed_dim, task_no)
        # 尺寸变化操作
        ...
        # 获取ecb21
        self.up1 = UpLayer(embed_dim // 2, embed_dim // 4)
        self.compress1 = ConvBlock(embed_dim, embed_dim // 2)
        # 获得dcb22
        self.sup2 = Sup(p, spec, 1, embed_dim // 2, task_no)

        # 获得dcb13
        self.sup3 = Sup(p, spec, 2, embed_dim // 4, task_no)
        ...
        self.up2 = UpLayer(embed_dim // 4, embed_dim // 8)
        self.compress2 = ConvBlock(embed_dim // 2, embed_dim // 4)
        self.up3 = UpLayer(embed_dim // 4, embed_dim // 8)
    ...
    def forward(self, x_dict, inter_pred, back_fea, ori_feature):
        ...
        dcb21 = self.up1(ecb20, self.compress1(ecb30))
        temp_feature = [ecb20 + dcb21 + dcb31[i] for i in range(self.task_no)]
        # 获得dcb22
        dcb22 = self.sup2(temp_feature)
        dcb22_ori = dcb22
        # 对dcb22 进行尺寸的扩大
        ...
        ecb10 = back_fea[0]
        # dcb11
        dcb11 = self.up2(ecb10, self.compress2(ecb20))
        dcb12 = self.up3(dcb11, self.compress3(dcb21))
        temp_feature = [dcb11 + ecb10 + dcb12 + dcb22[i] for i in range(self.task_no)]
        dcb13 = self.sup3(temp_feature)
        x_dict = {}
