...
from models.transformers.my_decoder import My_decoder
...
class TransformerDecoder(nn.Module):
    def __init__(self, p):
        ...
        self.scale_embed.append(nn.ConvTranspose2d(p.backbone_channels[0], embed_dim_with_pred // 4, kernel_size=8, stride=4, padding=2))
        self.scale_embed.append(nn.ConvTranspose2d(p.backbone_channels[1], embed_dim_with_pred // 2, kernel_size=3, stride=2, padding=1,output_padding=1))
        ...
        # 最终的解码器
        self.fin_decoder = My_decoder(p, in_chans=embed_dim_with_pred)
        pass
    ...
    def forward(self, x_list):
        ...
        x_dict = self.fin_decoder(ms_feat_dict, inter_pred, back_fea, ori_feature)
