...
class Fin_Decoder(nn.Module):

    def __init__(self, p, in_chans):
        super(Fin_Decoder, self).__init__()
        self.p = p
        ...
        self.Pself1 = Pself_Attention(p, spec, 0, embed_dim//2, task_no)
        self.Pself2 = Pself_Attention(p, spec, 1, embed_dim // 4, task_no)
        self.Pself3 = Pself_Attention(p, spec, 2, embed_dim // 8, task_no)
        self.compass1 = nn.ModuleList()
        self.compass2 = nn.ModuleList()
        for i in range(self.task_no):
            self.compass1.append(ConvBlock(2048, 1024))
            self.compass2.append(ConvBlock(1024,512))
            pass
        self.compass_1 = ConvBlock(1024, 512)
        self.compass_2 = ConvBlock(512, 256)
        self.compass_3 = ConvBlock(512, 256)
        self.up1 = UpLayer(512, 512 // 2)
        self.up2 = UpLayer(256, 256 // 2)
        self.up3 = UpLayer(256, 256 // 2)
        ...
        for i, task in enumerate(self.all_tasks):
            update1 = ConvBlock(1024, 256)
            update2 = ConvBlock(512, 256)
    ...
    def forward(self, x_dict, inter_pred, back_fea, ori_feature):
        ...
        ecb40 = x_list
        ecb30 = back_fea[-2]
        temp_feature = [self.compass1[i](ecb40[i]) + ecb30 for i in range(self.task_no)]
        temp_feature = self.Pself1(temp_feature)
        print(temp_feature[0].shape)
        dcb31 = temp_feature
        dcb31_ori = dcb31
        ecb20 = back_fea[1]
        dcb21 = self.up1(ecb20, self.compass_1(ecb30))
        print(dcb21.shape)
        temp_feature = [ecb20 + dcb21 + self.compass2[i](dcb31[i]) for i in range(self.task_no)]
        dcb22 = self.Pself2(temp_feature)
        dcb22_ori = dcb22
        dcb22 = [self.patch_embed[i](dcb22[i]) for i in range(self.task_no)]
        print(dcb22[0].shape)
        ecb10 = back_fea[0]
        # dcb11
        dcb11 = self.up2(ecb10, self.compass_2(ecb20))
        dcb12 = self.up3(dcb11, self.compass_3(dcb21))
        ...
        for i, task in enumerate(self.all_tasks):
            h, w = dcb13[i].shape[2:]
            dcb31_ori[i] = self.update1[i](
                F.interpolate(dcb31_ori[i], size=(h, w), mode='bilinear', align_corners=False))
            dcb22_ori[i] = self.update2[i](
                F.interpolate(dcb22_ori[i], size=(h, w), mode='bilinear', align_corners=False))
...
class Pself_Attention(nn.Module):
    def __init__(self, p, spec, i, dim_in, fea_no):
        super(Pself_Attention, self).__init__()
