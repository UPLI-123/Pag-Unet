...
def get_backbone(p):
    ...
    elif p['backbone'] == 'swinB':
        from models.transformers.swin import swin_b
        backbone = swin_b(pretrained=True)
        backbone_channels = [1024 for _ in range(4)]  # 人为划分为4个通道
        p.backbone_channels = backbone_channels
        # 将其划分为16块
        # p.spatial_dim = [[p.TRAIN.SCALE[0] // 16, p.TRAIN.SCALE[1] // 16] for _ in range(4)]
...
def get_head(p, backbone_channels, task):
    ...
    if p['head'] == 'mlp':
        from models.transform_net import MLPHead
        return MLPHead(128, p.TASKS.NUM_OUTPUT[task])
...
def get_model(p):
    ...
    if p['model'] == 'TransformerNet':
        ...
        model = TransformNet(p, backbone, backbone_channels, heads)
    else:
