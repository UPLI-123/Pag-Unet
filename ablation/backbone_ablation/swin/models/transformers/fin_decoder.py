...
class FinalDecoder(nn.Module):
    def __init__(self, p, in_chans):
        ...
        self.Pself1 = PselfAttention(p, spec, 0, 1024, task_no)
        self.Pself2 = PselfAttention(p, spec, 1, 256, task_no)
        self.Pself3  = PselfAttention(p, spec, 2, 128, task_no)

        self.pag1 = Pag(256, 256 // 2)
        self.pag2 = Pag(128, 128 // 2)
        self.pag3 = Pag(128, 128 // 2)

        self.compress1 = ConvBlock(512, 256)
        self.compress2 = ConvBlock(256, 128)
        self.compress3 = ConvBlock(256, 128)
        ...
        for i, task in enumerate(self.all_tasks):
            update1 = ConvBlock(512, 128)
            update2 = ConvBlock(256, 128)
    ...
    def forward(self, x_dict, inter_pred, back_fea, ori_feature):
        ...
        ecb30 = back_fea[-2]
        temp_feature = [self.patch_embed1[i](ecb40[i]) + ecb30 for i in range(self.task_no)]
        dcb31 = temp_feature
        dcb31_ori = dcb31
        # 将dcb31的尺寸进行扩大
        dcb31 = [self.patch_embed2[i](dcb31[i]) for i in range(self.task_no)]
        ecb20 = back_fea[1]
        ...
        dcb22_ori = dcb22
        # 对dcb22 进行尺寸的扩大
        dcb22 = [self.patch_embed3[i](dcb22[i]) for i in range(self.task_no)]
        # ecb10
        ecb10 = back_fea[0]
        # dcb11
